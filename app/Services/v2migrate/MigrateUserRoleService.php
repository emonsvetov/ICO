...
class MigrateUserRoleService extends MigrationService
{
    ...
    public function migrate($v2User, $v3User)
    {
        ...
        if (sizeof($v2UserRoles) > 0) {
            ...
            if ((!$v3User->organization_id || $v3User->organization_id == 1000000000)) {
                if ($this->v2pid()) {
                    if ($v2User->parent_program_id) {
                        $parentProgram = (new Program)->select(['id', 'organization_id'])->find($v2User->parent_program_id);
                        if ($parentProgram) {
            ...
            if ($this->v2pid() && $newProgramRoles) {
                ...
                foreach ($newProgramRoles as $programId => $programRoles) {
                    ...
                    if (!$v3User->organization_id || $v3User->organization_id == 1000000000) {
                        $parentProgram = (new Program)->select(['id', 'organization_id'])->find($programId);
                        if ($parentProgram) {
                            $v3User->organization_id = $parentProgram->organization_id;
                            $v3User->save();
        ...
        if ($v3User->organization_id == 1000000000) {
            if ($this->v2pid()) {
                $v2Parent = (new MigrateProgramsService)->get_program_info($this->v2pid());
                $parentProgram = $v2Parent ? (new Program)->select(['id', 'organization_id'])->find($v2Parent->v3_program_id) : null;
                if ($parentProgram) {
                    $v3User->organization_id = $parentProgram->organization_id;
                    $v3User->save();
                    $this->printf(" - OrganizationID \"%s\" set for v3user:\"%d\" from \"%s\".\n", $v3User->organization_id, $v3User->id, "v2User->parent_program_id");
                } else {
                    throw new Exception("Program not Found " . $v2Parent->v3_program_id);
                }
            } else {
                $v3User->organization_id = 1; //Super admin
                $v3User->save();
